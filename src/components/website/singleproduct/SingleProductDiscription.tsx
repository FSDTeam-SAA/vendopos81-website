import { Product, ProductVariant, WholesaleItem } from "@/lib/types/product";
import { Box, CheckCircle2, Container, Package } from "lucide-react";
...
const SingleProductDiscription = ({ data }: { data?: Product }) => {
  ...
  const showVariants =
    !showWholesale && data.variants && data.variants.length > 0;

  return (
    <section className="space-y-8 animate-in fade-in duration-500 ">
      {/* 1. Main HTML Description */}
      <div className="prose prose-stone max-w-none text-gray-600">
        <h3 className="text-xl font-semibold text-gray-900 mb-4 not-prose">
          Product Description
        </h3>
        <div
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-12">
          ...
          <SpecRow
            label="Stock Status"
            value={data.isAvailable ? "In Stock" : "Out of Stock"}
          />
          <SpecRow
            label="Brand"
            value={
              typeof data.supplierId === "object"
                ? data.supplierId?.brandName
                : "N/A"
            }
          />
        </div>
        ...
        <>
          ...
          <div>
            ...
            <div className="space-y-4">
              {/* Wholesale List */}
              {showWholesale &&
                data.wholesaleId?.map((item: WholesaleItem) => (
                  <div
                    key={item._id}
                    ...
                    <div>
                      ...
                      {item.type === "pallet" && item.palletItems?.[0] && (
                        <p className="text-sm text-gray-500">
                          ...
                          <span className="font-medium text-primary">
                            ${item.palletItems[0].price}
                          </span>
                          {item.palletItems[0].totalCases &&
                            ` • Cases: ${item.palletItems[0].totalCases}`}
                        </p>
                      )}
                      {item.type === "case" && item.caseItems?.[0] && (
                        <p className="text-sm text-gray-500">
                          Price:{" "}
                          <span className="font-medium text-primary">
                            ${item.caseItems[0].price}
                          </span>
                          {item.caseItems[0].quantity &&
                            ` • Qty: ${item.caseItems[0].quantity}`}
                        </p>
                      )}
                    </div>
                  </div>
              ...
              {/* Variants List */}
              {showVariants &&
                data.variants?.map((item: ProductVariant) => (
                  <div
                    key={item._id}
...
const SpecRow = ({
  ...
  <div className="flex justify-between items-center sm:grid sm:grid-cols-3 py-2 border-b border-gray-100 last:border-0">
    ...
    <span className="text-gray-900 font-medium text-right sm:text-left sm:col-span-2">
      {value || "—"}
    </span>
  </div>
);
