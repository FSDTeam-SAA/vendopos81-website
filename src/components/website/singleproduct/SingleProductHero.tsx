...
export default function SingleProductHero({ product }: { product: Product }) {
  ...
  const getPrice = (option: SelectedOption | null): number => {
    ...
    if ("type" in option) {
      ...
      if (wItem.type === "pallet" && wItem.palletItems?.[0])
        return wItem.palletItems[0].price || 0;
      if (wItem.type === "case" && wItem.caseItems?.[0])
        return wItem.caseItems[0].price || 0;
      return 0;
    }
              ...
              {images.map((img: any, index: number) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "relative w-20 h-20 rounded-lg bg-white transition-all",
                      isActive
        ...
        <div className="space-y-6">
          ...
          <p className="text-gray-600 leading-relaxed max-w-lg">
            {safeProduct.shortDescription ||
              safeProduct.description?.slice(0, 150) ||
                ...
                {variantList.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => setSelectedOption(item)}
                      className={cn(
                        // Strict TS might need item as SelectedOption cast if inferred wrong, but union should hold
                        "relative flex items-center justify-between p-3 rounded-xl border text-left transition-all",
                        isSelected
                          ? "border-primary bg-primary/5 ring-1 ring-primary"
                          : "border-gray-200 hover:border-gray-300",
          ...
          <div className="flex gap-4">
            ...
            <div>
              <Button
                ...
                onClick={handleClick}
                disabled={isPending || !safeProduct.isAvailable}
              >
            ...
            <div>
              <Button
                ...
                size="lg"
                className="px-4"
                onClick={handelClickForWishlist}
              >
                <Heart
                  size={20}
                  className={cn(false ? "fill-red-500 text-red-500" : "")}
                />
              </Button>
          ...
          <div className="pt-6 space-y-2 text-sm border-t border-gray-100">
            ...
            <div className="flex justify-between py-1 max-w-xs">
              ...
              <span
                className={cn(
                  "font-medium",
                  safeProduct.isAvailable ? "text-green-600" : "text-red-600",
                )}
              >
